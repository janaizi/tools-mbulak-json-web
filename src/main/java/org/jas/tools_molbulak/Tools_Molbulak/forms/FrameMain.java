...
public class FrameMain extends JFrame {
	 ...
	 */
	public FrameMain() {
		String jsontxt = JsonUrl.getUrlAsString("https://fs.molbulak.com/progs.txt");
					...
					public void run() {
						...
						if (val1.size() != 0) {
							...
							for (int k = 0; k < val1.size(); k++) {
								...
								String command = "mkdir C:\\SoftsMolbulak | bitsadmin /transfer softsMolbulak " + url
										+ " " + folder + file;
								System.out.println("url - " + url);
								System.out.println("folder - " + folder);
								RunCMD.run(command,"========= Downloaded "+file+"!","=== Error Download "+file+"!","Transfer complete.");
								System.out.println();
							}
							...
							for (int k = 0; k < val1.size(); k++) {
								...
								String folder = "C:\\SoftsMolbulak\\";
								String file = "" + url.subSequence((url.lastIndexOf('/') + 1), url.length());
								String command = val2.get(k).toString() + " " + folder + file;
								System.out.println("command - " + command);
								// RunCMD.run(command,"========= Download "+file+" OK!","=== Error Download "+file+"!");
							}
